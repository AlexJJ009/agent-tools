...
When invoked outside the already configured ReadPapers project, return a concrete handoff preserving the source locator and requested operation. Do not offer to make the current repository eligible by creating a config or redefining the project root. Existing supplied evidence remains usable locally without invoking this adapter.

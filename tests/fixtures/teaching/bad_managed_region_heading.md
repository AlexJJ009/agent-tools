---
title: Bad managed region heading fixture
---

%%zt-managed%%
## Teaching Reconstruction
ZotLit owns this heading and it must not trigger durable teaching artifact edits.
%%/zt-managed%%

<!-- teaching-reconstruction:v1 -->

```json teaching-manifest
{
  "schema_version": "teaching-reconstruction:v1",
  "artifact_id": "fixture-bad-managed-region-heading",
  "mode": "guided",
  "goal": {"target_ability": "Reconstruct the fixture from traceable evidence.", "success_criteria": ["Explain the target ability", "Pass the linked learning checks"]},
  "learner_state": {"profile_sources": ["reader_profile/index.md", "reader_profile/knowledge_map.md"], "assumptions": ["Fixture learner state is explicit."], "observed_gaps": []},
  "teaching_unit": {"kc": null, "explanation": "This unit binds one ability to evidence and a worked step.", "worked_step": "Follow the worked example, then answer from memory.", "next_action": "Run the linked learning check."},
  "knowledge_components": [],
  "teaching_edges": [],
  "frontier": [],
  "evidence_anchors": [],
  "learning_checks": [],
  "review_queue": []
}
```
